...
if [ "${1}" == "-h" ] || [ $# -lt 2 ]; then
	echo "Usage: backupe_zope_users_plain.sh <acronym> <backup-directory>"
...
URL="http://localhost:8080/Select/install/export_users_plain_py?kurztext=${NAME}"
FILE="${BACKUPDIR}/${HOSTNAME}_IHS-${NAME}_acl_users_${DATE}.txt"
if [ $VERBOSE ]; then
